...
## PASSA PRO OMBRO PARTE DO GIRO QUE SOBROU NO BRACO.
##
## O IK acima resolve DOIS ossos e deixa o terceiro — a clavicula — exatamente
## onde a animacao a pos. Numa pose de pistola isso passa: a mao fica na frente
## do peito e o braco gira pouco. Numa arma longa nao: a mao esquerda atravessa
## o corpo ate' o cano, e o braco esquerdo chega a ficar CEM GRAUS fora do
## descanso em relacao a' clavicula (medido: 81 parado, 111 correndo).
##
## Malha com skinning linear nao sobrevive a isso. A pele da omoplata esta'
## presa a' clavicula e ao braco, e um giro desse tamanho entre os dois a
## enrola — o "embrulho de bala". Foi o que apareceu nas costas do Maycow
## quando ele corria, e o motivo de sumir parado: parado o angulo cai pra 81.
##
## O conserto e' anatomico, nao cosmetico: quando alguem estica o braco pro
## outro lado do corpo, a OMOPLATA vai junto. Entao o ombro leva uma fracao do
## giro e o braco fica com o resto.
##
## O giro sai do proprio resultado do IK, entao nao ha' numero a adivinhar: e'
## `fracao` do que o braco de fato precisou, com um teto em graus pra nao virar
## encolher de ombros. Chame LOGO DEPOIS do `braco()` e rode o `braco()` de
## novo: mexer na clavicula muda de onde o braco sai, e a segunda passada e'
## que devolve a mao pro lugar exato.
static func aliviar_ombro(sk: Skeleton3D, i_braco: int, fracao: float,
		limite_graus: float) -> void:
	if fracao <= 0.001:
		return
	var i_ombro := sk.get_bone_parent(i_braco)
	if i_ombro < 0:
	...
	# O quanto o braco esta' torto EM RELACAO AO DESCANSO, no espaco da
	# clavicula (por isso a multiplicacao pela esquerda: e' o quadro do pai).
	var descanso := sk.get_bone_rest(i_braco).basis.get_rotation_quaternion()
	var agora := sk.get_bone_pose(i_braco).basis.get_rotation_quaternion()
	var delta := (agora * descanso.inverse()).normalized()
	# Quaternion com w negativo e' o mesmo giro pelo caminho longo: sem isto o
	# ombro daria a volta por tras pra chegar no mesmo lugar.
	if delta.w < 0.0:
		delta = -delta
	var graus := rad_to_deg(2.0 * acos(clampf(delta.w, -1.0, 1.0)))
	if graus < 0.5:
		return

	var parte := minf(fracao, limite_graus / graus)
	var pedaco := Quaternion.IDENTITY.slerp(delta, parte)
	var ombro := sk.get_bone_pose(i_ombro).basis.get_rotation_quaternion()
	sk.set_bone_pose_rotation(i_ombro, (ombro * pedaco).normalized())
